...
#define IS_AT(who, where)                                    \
  TEST(gv_get_location(gv, (who)) == (where),                \
       "gv_get_location(gv, %s) == %s %s", player_to_s(who), \
       location_get_abbrev(where), location_get_name(where))
#define HISTORY_IS(who, n, where)                                     \
  TEST(history[(n)] == (where), "gv_get_history(gv, %s): [%d] %s %s", \
...
int main(void) {
  ...
  do {  ////////////////////////////////////////////////////////////////
    char *symbols[] = {
        "gv_new",          "gv_drop",        "gv_get_round",
        "gv_get_player",   "gv_get_score",   "gv_get_health",
        "gv_get_location", "gv_get_history", "gv_get_connections"};
  ...
  do {  ////////////////////////////////////////////////////////////////
    ...
    TEST(gv_get_round(gv) == 0, "gv_get_round(gv) == 0");
    ...
    TEST(gv_get_score(gv) == GAME_START_SCORE, "gv_get_score(gv) == 366");
    IS_AT(PLAYER_LORD_GODALMING, UNKNOWN_LOCATION);

    gv_drop(gv);
    TEST(mem_address_is_poisoned(gv), "gv is no longer a valid allocation");
  } while (0);
  ...
  do {  ////////////////////////////////////////////////////////////////
    ...
    TEST(gv_get_player(gv) == PLAYER_LORD_GODALMING,
         "gv_get_player(gv) == Godalming");
    TEST(gv_get_round(gv) == 1, "gv_get_round(gv) == 1");
  ...
  do {  ////////////////////////////////////////////////////////////////
    ...
    do {
      ...
      location_t *edges = gv_get_connections(
          gv, &n_edges, GALATZ, PLAYER_LORD_GODALMING, 0, true, false, false);
    ...
    do {
      size_t n_edges;
      location_t *edges =
          gv_get_connections(gv, &n_edges, IONIAN_SEA, PLAYER_LORD_GODALMING, 0,
                             false, false, true);
    ...
    do {
      size_t n_edges;
      location_t *edges = gv_get_connections(
          gv, &n_edges, ATHENS, PLAYER_LORD_GODALMING, 0, false, true, false);
  ...
  do {  ////////////////////////////////////////////////////////////////
    ...
    gv_drop(gv);
    TEST(mem_address_is_poisoned(gv), "hv is no longer a valid allocation");
  } while (0);
