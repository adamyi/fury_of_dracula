 ...
 *     gcc -DI_AM_DRACULA -o dracula player.c dracula.c dracula_view.c ... -ljansson
 * where ... includes GameView.c along with any other player's files.
 *
...
int main (void)
{
	...
	view_t state = view_new (
		ps.input_data.past_plays,
		ps.input_data.messages
	);
	decide_move (state);
	view_drop (state);
...
static void get_input (struct input_data *input_data)
{
	...
	if (rootJSON == NULL)
		errx (EX_DATAERR, "input error: on line %d: %s",
	...
	for (size_t i = 0; i < messages_len; i++) {
		...
		if (!json_is_string (message_item_json))
			errx (EX_DATAERR,
				"input error: $.messages[%zu] is not a string", i);
