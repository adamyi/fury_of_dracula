...
TEST(encounterTest, drS_encounters_drac_and_vamp_then_drac_and_trap) {
  ...
  ac_compare_int((int)gv_get_health(gv, PLAYER_DR_SEWARD), 0,
                 "Dr Steward has correct life (0)");
  ac_compare_int((int)gv_get_health(gv, PLAYER_DRACULA),
                 GAME_START_BLOOD_POINTS - 20,
                 "gv_get_health(gv, Dracula) == 20 (start - 2*encounter)");
  ac_compare_int((int)gv_get_score(gv), GAME_START_SCORE - 4 - 6,
...
TEST(hospitalTest, drS_teleports_to_hospital_drac_sea_dmg) {
  ...
  ac_compare_int(
      (int)gv_get_health(gv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS - 20 - 2,
      "gv_get_health(gv, Dracula) == 20 (start - 2*encounter - 1*sea)");
  ac_compare_int((int)gv_get_score(gv), GAME_START_SCORE - 5 - 6,
                 "gv_get_score(gv) == 366 - 10");
  gv_drop(gv);
}
...
TEST(castleDracTest, drac_heals_in_castle) {
  ...
  ac_compare_int((int)gv_get_score(gv), GAME_START_SCORE - 11 - 6,
                 "gv_get_score(gv) == 366 - 17");
  ac_compare_int((int)gv_get_location(gv, PLAYER_DRACULA), (int)CASTLE_DRACULA,
...
TEST(castleDracTest, drac_heals_in_castle_to_max) {
  ...
  ac_compare_int((int)gv_get_health(gv, PLAYER_DRACULA), 72,
                 "Dracula has correct life (72) (no-max)");
  ac_compare_int((int)gv_get_health(gv, PLAYER_LORD_GODALMING),
                 GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int((int)gv_get_health(gv, PLAYER_DR_SEWARD),
                 GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int((int)gv_get_health(gv, PLAYER_VAN_HELSING),
                 GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int((int)gv_get_health(gv, PLAYER_MINA_HARKER),
                 GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int((int)gv_get_score(gv), GAME_START_SCORE - 16 - 6,
                 "gv_get_score(gv) == 366 - 22");
  ac_compare_int((int)gv_get_location(gv, PLAYER_DRACULA), (int)CASTLE_DRACULA,
                 "Dracula is in the Castle");
  gv_drop(gv);
}
