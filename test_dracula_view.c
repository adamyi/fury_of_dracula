...
TEST(encounterTest, drS_encounters_drac_and_vamp){
  ...
  ac_compare_int(dv_get_health(dv, PLAYER_DR_SEWARD), GAME_START_HUNTER_LIFE_POINTS - 4, "Dr Steward has correct life (5)");
  ac_compare_int(dv_get_health(dv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS - 10, "dv_get_health(dv, Dracula) == 30 (start - encounter)");
  ac_compare_int(dv_get_score(dv), GAME_START_SCORE - 3, "dv_get_score(dv) == 366 - 3");
  ac_compare_int(dv_get_location(dv, PLAYER_DR_SEWARD), MEDITERRANEAN_SEA, "Dr S is in Med Sea");
  dv_drop(dv);
}
...
TEST(encounterTest, drS_encounters_drac_and_vamp_then_drac_and_trap){
  ...
  ac_compare_int(dv_get_health(dv, PLAYER_DR_SEWARD), 0, "Dr Steward has correct life (0)");
  ac_compare_int(dv_get_health(dv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS - 20, "dv_get_health(dv, Dracula) == 20 (start - 2*encounter)");
  ac_compare_int(dv_get_score(dv), GAME_START_SCORE - 4 - 6, "dv_get_score(dv) == 366 - 10");
  dv_drop(dv);
}
...
TEST(hospitalTest, drS_teleports_to_hospital_drac_sea_dmg){
  ...
  ac_compare_int(dv_get_health(dv, PLAYER_DR_SEWARD), GAME_START_HUNTER_LIFE_POINTS, "Dr Steward has correct life (9)");
  ac_compare_int(dv_get_health(dv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS - 20 - 2, "dv_get_health(dv, Dracula) == 20 (start - 2*encounter - 1*sea)");
  ac_compare_int(dv_get_score(dv), GAME_START_SCORE - 5 - 6, "dv_get_score(dv) == 366 - 10");
  dv_drop(dv);
}
...
TEST(castleDracTest, drac_heals_in_castle){
  ...
  ac_compare_int(dv_get_health(dv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS - 20 - 8 + 10, "Dracula has correct life (22)");
  ac_compare_int(dv_get_score(dv), GAME_START_SCORE - 11 - 6, "dv_get_score(dv) == 366 - 17");
  ac_compare_int(dv_get_location(dv, PLAYER_DRACULA), CASTLE_DRACULA, "Dracula is in the Castle");
...
TEST(castleDracTest, drac_heals_check_traps_check_locs) {
  ...
  ac_compare_int(dv_get_health(dv, PLAYER_DRACULA), 60, "Dracula has correct life (60) (no-max)");
  ac_compare_int(dv_get_health(dv, PLAYER_LORD_GODALMING), GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int(dv_get_health(dv, PLAYER_DR_SEWARD), GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int(dv_get_health(dv, PLAYER_VAN_HELSING), GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int(dv_get_health(dv, PLAYER_MINA_HARKER), GAME_START_HUNTER_LIFE_POINTS, "player has full health");
  ac_compare_int(dv_get_score(dv), GAME_START_SCORE - 2, "gv_get_score(dv) == 366 - 2");
  ac_compare_int(dv_get_location(dv, PLAYER_DRACULA), CASTLE_DRACULA, "Dracula is in the Castle");
  int n_traps = 0;
  int n_vamps = 0;
  ...
  location_t *dests =  dv_get_dests_player(dv, &n_locations, PLAYER_DRACULA, true, true, true);
