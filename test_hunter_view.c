...
TEST(connsTest, test_score_start_HP_and_sea_cons) {
  char *trail = "GED.... SGE.... HZU.... MCA.... DC?.V.." // 0
  ...
  n_locations = 0;
  location_t *lordGAllDests = hv_get_dests(hv, &n_locations, true, true,
                                          true);
  memset(sea_destsLG_ret, false, NUM_MAP_LOCATIONS);
  for(size_t i = 0; i < n_locations; i++){
  ...
  location_t *minaRailLocs =  hv_get_dests_player(hv, &n_locations,
                                                  PLAYER_MINA_HARKER, false, true, false);
  ...
  n_locations = 0;
  location_t *minaAllLocs =  hv_get_dests_player(hv, &n_locations,
                                                PLAYER_MINA_HARKER,
                                                true, true, true);
...
TEST(dracTrailTest, drac_teleports_from_castle_double_back_to_hide) {
  ...
  ac_compare_int(hv_get_health(hv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS +
                3*LIFE_GAIN_CASTLE_DRACULA, "Drac HP = 70");

  ac_compare_int(hv_get_location(hv, PLAYER_DRACULA), CASTLE_DRACULA, "location"
                " CASTLE_DRACULA");
...
TEST(dracMoveTest, test_get_dests_player_on_drac){
  ...
  size_t n_locations = 0;
  location_t *dracDests = hv_get_dests_player(hv, &n_locations, PLAYER_DRACULA,
                                            true, true, true);
...
TEST(hospitalTest, drS_teleports_to_hospital) {
  ...
  ac_compare_int(hv_get_round(hv), 4, "round is 4");
  ac_compare_int(hv_get_health(hv, PLAYER_DR_SEWARD),
                 0, "Dr Steward has correct life (9)");
  ac_compare_int(hv_get_health(hv, PLAYER_DRACULA), GAME_START_BLOOD_POINTS -
                20, "hv_get_health(hv, Dracula) == 20 (start - 2*encounter)");
