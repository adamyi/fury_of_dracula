...
map *map_new(void) {
  ...
  (*g) = (map){
      .n_vertices = NUM_MAP_LOCATIONS, .n_edges = 0, .connections = {NULL}};
  ...
  for (size_t i = 0; i < g->n_vertices; i++) {
    for (map_adj *curr = g->connections[i]; curr != NULL; curr = curr->next) {
      printf("%s connects to %s by %s\n", location_get_name((location_t)i),
             location_get_name(curr->v), transport_to_s(curr->type));
    }
  }
}
...
size_t map_ne(map *g, transport_t type) {
  ...
  for (size_t i = 0; i < g->n_vertices; i++) {
    for (map_adj *curr = g->connections[i]; curr != NULL; curr = curr->next)
      if (curr->type == type || type == ANY) n_edges++;
  }
...
static void add_connections(map *g) {
  ...
  for (size_t i = 0; !is_sentinel_edge(CONNECTIONS[i]); i++)
    add_connection(g, CONNECTIONS[i].v, CONNECTIONS[i].w, CONNECTIONS[i].t);
}

/// Add a new edge to the Map/Graph
static void add_connection(map *g, location_t start, location_t end,
                           transport_t type) {
  assert(g != NULL);
  assert(start != end);
...
static bool adjlist_contains(map_adj *list, location_t v, transport_t type) {
  ...
  for (map_adj *curr = list; curr != NULL; curr = curr->next)
    if (curr->v == v && curr->type == type) return true;
  return false;
}
